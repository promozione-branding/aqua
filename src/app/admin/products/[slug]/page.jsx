...
export default function EditProductPage() {
 ...
 const handleSetPrimaryVariant = (index) => {
  const updatedVariants = colorVariants.map((variant, i) => {
    if (i === index) {
      return {
        ...variant,
        primary: !variant.primary, // ✅ toggle true/false
      };
    }
    return variant; // ✅ keep others unchanged
  });
  ...
  const handleImageChange = async (index, e) => {
    ...
    if (files.length > 0) {
      try {
        toast.loading("Converting and compressing images...", {
          id: "webp-convert",
        });
        ...
        setColorVariants(updated);
        toast.success("Images compressed and converted to WebP successfully", {
          id: "webp-convert",
        });
      } catch (error) {
  ...
  const removeVariantImage = (variantIndex, imageIndex) => {
    ...
    const previewUrl =
      typeof previewItem === "object" ? previewItem.url : previewItem;
    ...
    if (previewUrl && previewUrl.startsWith("blob:")) {
      ...
      const oldImageIndex = updated[variantIndex].existingImages?.findIndex(
        (img) => img.url === previewUrl,
      );
        ...
        <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-lg p-8">
          <div className="mb-8 text-center">
            ...
            <p className="text-gray-500 mt-2">
              Modify product pricing, categories, specifications, and variants
            </p>
          </div>
              ...
              {colorVariants.map((variant, index) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <button
  type="button"
  onClick={() => handleSetPrimaryVariant(index)}
  className="text-2xl"
>
  {variant.primary ? "⭐" : "☆"}
</button>
                  ...
                  <label className="border-2 border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-black transition">
                    ...
                    <input
                      type="file"
                      multiple
                      accept="image/*, .heic, .heif, .webp, .png, .jpg, .jpeg, .gif"
                  ...
                  {variant.previews.length > 0 && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {variant.previews.map((previewItem, i) => {
                        ...
                        const origSize =
                          isObj && previewItem.originalSize
                            ? (previewItem.originalSize / 1024 / 1024).toFixed(
                                2,
                              ) + " MB"
                            : null;
                        const newSize =
                          isObj && previewItem.newSize
                            ? (previewItem.newSize / 1024).toFixed(1) + " KB"
                            : null;
                          ...
                          >
                            ...
                            {origSize && newSize && (
                              <div className="absolute top-0 left-0 bg-black/75 backdrop-blur-sm p-1.5 rounded-br-lg text-[9px] font-mono text-white leading-tight shadow-md border-r border-b border-white/10 z-10">
                                ...
                                <div className="text-[#34d399] font-bold mt-0.5">
                                  WEBP: {newSize}
                                </div>
                              </div>
                            )}
